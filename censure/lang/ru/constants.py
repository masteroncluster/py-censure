...
FOUL_CORE = {
    ...
    'еб#2': '(ебну)|([еи][б][ыу]р)|([(орж)|(от)|(по)|(под)|(при)|(про)|(раз)|(рас)|(у)][ъь]?[ие]б)|'
            '(е[б][лносуц])|(е.?[бп][аы]мат)',
    ...
    'перд': '([^су]перн((у)|(ет)))|(выпер[дж])|(пердан)|(^перд$)|(перд[ея][жлнт])|(перд[ия][млтщ])|'
            '(пердн[еу])|(перд((ли)|(ол)|(у)))|(перд[ью])|(пердо$)|([^су]п[еи]рну)',
    ...
    'сать': '(^сс?ать)|(ссат)|(^ссу$)|(ссут)|(^ссы$)|(^сс?ыку)|(^ссыте?$)|(^ссыш)|(ссан)|(((обо)|'
            '(пере)|(недо))сс?а[лт])',
    'сран': '(сран[н]?[адекоыь])|(сра[лч]ь?)|(срать)|(срет.?(ся)?$)|(^сри$)|(^сру$)|(срун)|(срут)|'
